...
const mergeClasses = (...parts: Array<string | undefined>) => parts.filter(Boolean).join(' ');

export function Button({ variant, className, ...props }: ButtonProps) {
  const variantClass = variant ? ui.button[variant] : undefined;
  return <button {...props} className={mergeClasses(variantClass, className)} />;
}
  ...
  const variantClass =
    variant === 'compact'
      ...
      : variant === 'training'
        ? ui.trainingInput
        : ui.select;
  return <select {...props} className={mergeClasses(variantClass, className)} />;
}
...
export function Field({
  ...
  const baseClass = layout === 'row' ? 'flex items-center gap-2' : 'flex flex-col gap-1';
...
export function EmptyState({ className, ...props }: EmptyStateProps) {
  return <div {...props} className={mergeClasses(ui.emptyBox, className)} />;
}
