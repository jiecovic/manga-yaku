...
export async function loadPageState(volumeId: string, filename: string): Promise<Box[]> {
  const res = await fetch(
    `${BASE}/api/boxes/${encodeURIComponent(volumeId)}/${encodeURIComponent(filename)}`,
    {
      headers: {
...
export async function patchBoxText(
  ...
  await apiFetch(
    `${BASE}/api/boxes/${encodeURIComponent(volumeId)}/${encodeURIComponent(filename)}/${boxId}`,
    {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      keepalive: options?.keepalive ?? false,
