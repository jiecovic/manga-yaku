...
export async function fetchLlmCallLog(id: string): Promise<LlmCallLogDetailResponse> {
  const res = await apiFetch(`${API_BASE}/api/logs/llm-calls/${encodeURIComponent(id)}`, {
    headers: { Accept: 'application/json' },
  });
