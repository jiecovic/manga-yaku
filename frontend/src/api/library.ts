...
export function fetchPages(volumeId: string): Promise<PageInfo[]> {
    return getJson<PageInfo[]>(
        `${API_BASE}/api/volumes/${encodeURIComponent(volumeId)}/pages`,
    );
}
...
export async function deleteVolumePage(
    ...
    const res = await fetch(
        `${API_BASE}/api/volumes/${encodeURIComponent(volumeId)}/pages/${encodeURIComponent(
            filename,
        )}`,
        {
            method: "DELETE",
...
export async function detectMissingPages(): Promise<MissingPage[]> {
    return getJson<MissingPage[]>(
        `${API_BASE}/api/volumes/sync/pages/missing`,
    );
}

export async function pruneMissingPages(
    pages: MissingPage[],
