...
import { createBoxDetectionJob, loadPageState, patchBoxText, savePageState } from '../api/boxes';
...
export interface UsePageBoxesResult {
  ...
  handleUpdateBoxText: (id: number, field: 'text' | 'translation', value: string) => void;
...
export function usePageBoxes(): UsePageBoxesResult {
  ...
  const pendingTextUpdatesRef = useRef<Record<number, { text?: string; translation?: string }>>({});
  ...
  const handleChangeBoxesForType = (type: BoxType, next: Box[]) => {
    ...
    const others = boxes.filter((box) => normalizeBoxType(box.type) !== normalizedType);
    updateBoxesAndPersist([...others, ...normalizedNext], true);
  };

  const handleDeleteBox = (type: BoxType, id: number) => {
    const normalizedType = normalizeBoxType(type);
    const next = boxes.filter((b) => !(normalizeBoxType(b.type) === normalizedType && b.id === id));
    updateBoxesAndPersist(next, true);
  };

  const handleClearBoxes = (type: BoxType) => {
    const normalizedType = normalizeBoxType(type);
    const next = boxes.filter((b) => normalizeBoxType(b.type) !== normalizedType);
    updateBoxesAndPersist(next, true);
  };

  const handleMoveBox = (type: BoxType, id: number, dir: 'up' | 'down') => {
    const normalizedType = normalizeBoxType(type);
    const layer = boxes.filter((b) => normalizeBoxType(b.type) === normalizedType);
  ...
  // -------------------------------
  const handleUpdateBoxText = (id: number, field: 'text' | 'translation', value: string) => {
    setPageBoxesLocal((prev) =>
      prev.map((b) =>
        b.id === id && normalizeBoxType(b.type) === 'text' ? { ...b, [field]: value } : b,
      ),
    );
    ...
    if (immediate) {
      ...
      const payload = field === 'text' ? { text: value } : { translation: value };
  ...
  const handleAutoDetectBoxes = async (profileId?: string, task?: string) => {
