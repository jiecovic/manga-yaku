...
export function usePageJobActions({
  ...
  const handlePageTranslationWorkflow = async () => {
    ...
    try {
      console.log(`Queuing PAGE TRANSLATION workflow for ${volumeId}/${filename}`);
      ...
      await createPageTranslationJob(
        {
          ...
          filename,
          detectionProfileId:
            pageTranslationDetectionProfileId || boxDetectionProfileId || undefined,
      ...
      console.error('Failed to queue page translation workflow for page', err);
    } finally {
  ...
  const handleOcrBox = async (id: number) => {
    ...
    const box = boxes.find((b) => b.id === id && normalizeBoxType(b.type) === 'text');
    if (!box) return;
    ...
    try {
      ...
      const res = await createOcrBoxJob({
        ...
        height: box.height,
        boxId: box.id,
        boxOrder: box.orderIndex && box.orderIndex > 0 ? box.orderIndex : undefined,
      });
