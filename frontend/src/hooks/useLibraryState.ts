...
function readLibrarySelection(): LibrarySelection {
  ...
  try {
    ...
    const volumeId = typeof parsed.volumeId === 'string' ? parsed.volumeId : '';
    const pageIndex =
...
function writeLibrarySelection(selection: LibrarySelection) {
  ...
  try {
    window.sessionStorage.setItem(LIBRARY_SELECTION_KEY, JSON.stringify(selection));
  } catch {
...
export function useLibraryState() {
  ...
  const [selectedVolumeId, setSelectedVolumeId] = useState<string>(initialSelection.volumeId);
  ...
  const [boxDetectionProfiles, setBoxDetectionProfiles] = useState<BoxDetectionProfile[]>([]);
  const [boxDetectionProfileId, setBoxDetectionProfileId] = useState<string>('');
  ...
  const [translationProviders, setTranslationProviders] = useState<TranslationProvider[]>([]);
  const [translationProfileId, setTranslationProfileId] = useState<string>('');
  ...
  const applyBoxDetectionProfiles = useCallback(
    (profiles: BoxDetectionProfile[], preserveSelection: boolean) => {
      setBoxDetectionProfiles(profiles);
      setBoxDetectionProfileId((prev) => {
        const hasPrev = prev ? profiles.some((p) => p.enabled && p.id === prev) : false;
  ...
  useEffect(() => {
    ...
    const loadTranslationProviders = async () => {
      try {
        ...
        setTranslationProfileId((currentId) => {
          if (currentId) {
            return currentId;
          }
          return providers.find((p) => p.enabled)?.id ?? '';
      ...
      try {
        ...
        setPageIndex((prev) => {
          ...
          if (volumeChanged) {
            ...
            if (!restore.consumed && restore.volumeId && restore.volumeId === selectedVolumeId) {
              restore.consumed = true;
              if (restore.filename) {
                const matchIndex = ps.findIndex((p) => p.filename === restore.filename);
                if (matchIndex >= 0) {
                  return matchIndex;
                }
              }
        ...
        if (message.includes('HTTP 404') || message.includes('Volume not found')) {
          setSelectedVolumeId('');
          setPages([]);
          setPageIndex(0);
  ...
  const addPageFromClipboard = async (
    ...
    try {
      ...
      const index = updatedPages.findIndex((p) => p.filename === page.filename);
      setPageIndex(index >= 0 ? index : Math.max(updatedPages.length - 1, 0));
      setVolumes((prev) =>
        prev.map((volume) =>
          volume.id === targetVolumeId
            ? {
                ...volume,
                pageCount: Math.max(volume.pageCount, updatedPages.length),
              }
            : volume,
        ),
      );
  ...
  const detectMissingVolumesInDb = async () => {
    const [volumes, pages] = await Promise.all([detectMissingVolumes(), detectMissingPages()]);
    return { volumes, pages };
  };

  const pruneMissingVolumesInDb = async (
    ids: string[],
  ...
  const deletePageFromVolume = async (volumeId: string, filename: string) => {
    ...
    try {
      ...
      setVolumes((prev) =>
        prev.map((volume) =>
          volume.id === volumeId ? { ...volume, pageCount: updatedPages.length } : volume,
        ),
      );
      setError(null);
    } catch (err) {
  ...
  const currentPage = useMemo(
    () => (pageIndex >= 0 && pageIndex < pages.length ? pages[pageIndex] : undefined),
    [pages, pageIndex],
  );
