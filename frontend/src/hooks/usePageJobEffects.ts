...
function readTrimmedString(source: Record<string, unknown> | null, keys: string[]): string | null {
  if (!source) {
    return null;
  }
...
function resolvePayloadPage(
  ...
  for (const candidate of candidates) {
    const resolvedVolumeId = readTrimmedString(candidate, ['volumeId', 'volume_id']);
    const resolvedFilename = readTrimmedString(candidate, ['filename', 'file_name']);
      ...
      setPageBoxesLocal((currentBoxes) => {
        ...
        const idx = Number.isFinite(payloadBoxId)
          ? currentBoxes.findIndex(
              (b) => b.id === payloadBoxId && normalizeBoxType(b.type) === 'text',
            )
    ...
    jobs.forEach((job) => {
      ...
      setPageBoxesLocal((currentBoxes) => {
        const idx = currentBoxes.findIndex(
          (b) => b.id === boxId && normalizeBoxType(b.type) === 'text',
        );
        if (idx === -1) {
          return currentBoxes;
        }
        ...
        if (target.translation && String(target.translation).trim().length > 0) {
          return currentBoxes;
        }
