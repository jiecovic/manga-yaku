...
export function usePageWorkspace({
  ...
  const { jobs } = useJobs();
  const [activeBoxTypeByPage, setActiveBoxTypeByPage] = useState<Record<string, BoxType>>({});
  ...
  const { handleOcrPage, handleOcrBox, handleTranslateBox, handlePageTranslationWorkflow } =
    usePageJobActions({
      boxes,
      ocrProfileId,
      translationProfileId,
  ...
  const toggleVisibleBoxType = (type: BoxType) => {
    setHiddenBoxTypes((prev) => {
      ...
      if (willHide && type === activeBoxType) {
        const fallback = EDITABLE_BOX_TYPES.find((item) => !next.has(item));
        if (fallback) {
  ...
  const pageDataProps: PageDataProps = {
    ...
    onMoveBox: (id: number, dir: 'up' | 'down') => handleMoveBox('text', id, dir),
  ...
  const pageActions: PageActions = {
    ...
    // NEW
    onAutoDetectBoxes: () => {
      void handleAutoDetectBoxes(boxDetectionProfileId || undefined, boxDetectionTask || undefined);
    },
    onRefreshPageState: () => {
