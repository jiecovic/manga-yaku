...
export function useJobLogs(jobId: string | null, maxLines: number = 400): JobLogsState {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const source = new EventSource(`${appConfig.apiBase}/api/jobs/${jobId}/logs/stream`);
    ...
    source.onmessage = (event) => {
      ...
      setState((prev) => {
        ...
        if (isProgress && next.length > 0 && isProgressLine(next[next.length - 1])) {
          next[next.length - 1] = event.data;
        } else {
          next.push(event.data);
        }
        const clipped = next.length > maxLines ? next.slice(next.length - maxLines) : next;
        return {
          jobId,
          lines: clipped,
          status: 'connected',
