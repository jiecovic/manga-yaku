...
export function coerceModelCapabilities(raw: unknown): Record<string, ModelCapability> {
  ...
  for (const [modelId, value] of entries) {
    ...
    result[modelId] = {
      model_id: typeof record.model_id === 'string' ? record.model_id : modelId,
      applies_temperature: Boolean(record.applies_temperature),
      applies_reasoning_effort: Boolean(record.applies_reasoning_effort),
      temperature_support:
        typeof record.temperature_support === 'string' ? record.temperature_support : 'always',
