...
export function useSettingsLayoutState() {
  const { settings, loading: baseLoading, error: baseError, save, refresh } = useSettings();
  const {
    pageTranslation,
    ocrProfiles,
    translationProfiles,
  ...
  const [pageTranslationDraft, setPageTranslationDraft] = useState<PageTranslationDraft | null>(
    null,
  );
  ...
  useEffect(() => {
    if (pageTranslation?.value) {
      ...
      setPageTranslationDraft({
        ...
        max_output_tokens:
          value.max_output_tokens === null || value.max_output_tokens === undefined
            ? ''
  ...
  useEffect(() => {
    if (translationProfiles?.profiles) {
      setTranslationDraft(translationProfiles.profiles.map((profile) => ({ ...profile })));
  ...
  const pageTranslationSelectedCapability = useMemo<ModelCapability>(
    () => resolveModelCapability(pageTranslationModelCapabilities, pageTranslationDraft?.model_id),
  ...
  const confThreshold = useMemo(() => draftString(draft, 'detection.conf_threshold'), [draft]);
  const iouThreshold = useMemo(() => draftString(draft, 'detection.iou_threshold'), [draft]);
  ...
  const includePriorContextSummary = useMemo(
    () => draftBoolean(draft, 'page_translation.include_prior_context_summary', true),
    [draft],
  );
  ...
  const agentChatMaxTurns = useMemo(() => draftString(draft, 'agent.chat.max_turns'), [draft]);
  const agentChatMaxOutputTokens = useMemo(
    () => draftString(draft, 'agent.chat.max_output_tokens'),
    [draft],
  );
  const mergeReasoningEffort = useMemo(() => {
    const raw = draftString(draft, 'page_translation.merge.reasoning_effort').trim().toLowerCase();
    if (raw === 'low' || raw === 'medium' || raw === 'high') {
      return raw;
    }
    return 'low';
  }, [draft]);
  const ocrParallelismLocal = useMemo(() => draftString(draft, 'ocr.parallelism.local'), [draft]);
  const ocrParallelismRemote = useMemo(() => draftString(draft, 'ocr.parallelism.remote'), [draft]);
  ...
  const ocrParallelDefaults = useMemo(
    () => ({
      local: toIntWithFallback(String(settings?.defaults?.['ocr.parallelism.local'] ?? '4'), 4),
      remote: toIntWithFallback(String(settings?.defaults?.['ocr.parallelism.remote'] ?? '2'), 2),
      ...
      taskTimeoutSeconds: toIntWithFallback(
        String(settings?.defaults?.['ocr.parallelism.task_timeout_seconds'] ?? '180'),
        180,
      ),
    }),
    [settings],
  ...
  const mergeDefaults = useMemo(
    () => ({
      maxOutputTokens: toIntWithFallback(
        String(settings?.defaults?.['page_translation.merge.max_output_tokens'] ?? '768'),
        768,
      ),
      reasoningEffort: (() => {
        const raw = String(settings?.defaults?.['page_translation.merge.reasoning_effort'] ?? 'low')
          .trim()
          .toLowerCase();
        if (raw === 'low' || raw === 'medium' || raw === 'high') {
          return raw;
  ...
  const chatDefaults = useMemo(
    () => ({
      maxTurns: toIntWithFallback(String(settings?.defaults?.['agent.chat.max_turns'] ?? '18'), 18),
      maxOutputTokens: toIntWithFallback(
  ...
  const buildBaseSettingsPayload = useCallback(
    () => ({
      ...
      'detection.containment_threshold': containmentThreshold ? Number(containmentThreshold) : null,
      ...
      'page_translation.include_prior_context_summary': includePriorContextSummary,
      ...
      'agent.chat.max_turns': toIntWithFallback(agentChatMaxTurns, chatDefaults.maxTurns),
      'agent.chat.max_output_tokens': toIntWithFallback(
        agentChatMaxOutputTokens,
        chatDefaults.maxOutputTokens,
      ),
      'ocr.parallelism.local': toIntWithFallback(ocrParallelismLocal, ocrParallelDefaults.local),
      'ocr.parallelism.remote': toIntWithFallback(ocrParallelismRemote, ocrParallelDefaults.remote),
  ...
  const pageTranslationDetectionOptions = useMemo(() => {
    ...
    const enabledProfiles = pageTranslationDetectionProfiles.filter((profile) => profile.enabled);
    ...
    if (
      pageTranslationDetectionProfileId &&
      !availableProfiles.some((profile) => profile.id === pageTranslationDetectionProfileId)
    ) {
      return [
        {
  ...
  const updateTranslationProfile = (id: string, updates: Partial<TranslationDraftProfile>) => {
  ...
  const buildOcrPayload = useCallback(
    () => ({
      profiles: ocrDraft.map((profile) => ({
        ...
        max_output_tokens:
          profile.max_output_tokens === null || profile.max_output_tokens === undefined
            ? null
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(SETTINGS_AUTOSAVE_KEY, autoSaveEnabled ? '1' : '0');
  }, [autoSaveEnabled]);
  ...
  }, [baseDirty, autoSaveEnabled, baseLoading, save, buildBaseSettingsPayload]);
  ...
  const handleRestartBackend = async () => {
    if (typeof window !== 'undefined') {
      const confirmed = window.confirm('Restart backend now? Active requests may be interrupted.');
      if (!confirmed) {
        return;
      }
    }
    ...
    try {
      ...
      const message =
        err instanceof Error && err.message ? err.message : 'Failed to request backend restart.';
