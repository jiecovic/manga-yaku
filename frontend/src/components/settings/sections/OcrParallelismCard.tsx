      ...
      <div className="mt-3 space-y-3">
        <Field label="Local" layout="row" labelClassName={ui.label}>
          <input
            ...
            onChange={(e) => onUpdateDraft('ocr.parallelism.local', e.target.value)}
          />
        </Field>
        ...
        <Field label="Remote" layout="row" labelClassName={ui.label}>
          <input
            ...
            onChange={(e) => onUpdateDraft('ocr.parallelism.remote', e.target.value)}
          />
        </Field>
        <div className={`${ui.trainingHelp} ml-28`}>
        ...
        <Field label="Max workers" layout="row" labelClassName={ui.label}>
          <input
            ...
            onChange={(e) => onUpdateDraft('ocr.parallelism.max_workers', e.target.value)}
          />
        </Field>
        <div className={`${ui.trainingHelp} ml-28`}>
        ...
        <Field label="Lease (s)" layout="row" labelClassName={ui.label}>
          <input
            ...
            onChange={(e) => onUpdateDraft('ocr.parallelism.lease_seconds', e.target.value)}
          />
        </Field>
        <div className={`${ui.trainingHelp} ml-28`}>
          Task claim validity window before stale running tasks can be re-queued.
        </div>
        <Field label="Timeout (s)" layout="row" labelClassName={ui.label}>
          <input
            ...
            onChange={(e) => onUpdateDraft('ocr.parallelism.task_timeout_seconds', e.target.value)}
          />
        </Field>
        <div className={`${ui.trainingHelp} ml-28`}>Hard runtime limit per OCR task attempt.</div>
        <div className={ui.trainingHelp}>
          Local = on-device OCR (manga-ocr). Remote = API OCR (LLM profiles).
        </div>
        <div className={ui.trainingHelp}>Effective workers = min(local + remote, max workers).</div>
      </div>
    </div>
  );
}
