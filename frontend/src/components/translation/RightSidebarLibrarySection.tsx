...
export function RightSidebarLibrarySection({
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            if (result.volumesImported === 0 && result.pagesImported === 0) {
                ...
                setSyncError(
                    `Imported ${result.volumesImported} volumes, ${result.pagesImported} pages.`,
                );
            }
        } catch (err) {
            if (err instanceof Error && err.message) {
    ...
    return (
        <CollapsibleSection title="Library" defaultOpen>
            {loadingVolumes && (
                <div className={ui.mutedTextXs}>Loading volumes...</div>
            )}
            ...
            <div className="flex items-center gap-2 w-full">
                ...
                <Button
                    ...
                    variant="icon"
                    className={
                        loadingPages || loadingVolumes
                            ? ui.button.iconDisabled
                            : undefined
                    }
                >
                    R
            ...
            {syncError && (
                <div className={`mt-2 ${ui.warningTextTiny}`}>
                    {syncError}
                </div>
            )}
                ...
                <div className={ui.modalOverlay}>
                    <div className={ui.modalPanel}>
                        ...
                        <div className={ui.modalText}>
                            These items are in the DB but missing on disk.
                        </div>
