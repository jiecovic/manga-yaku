...
export function PageCanvas({
  ...
  // Box drawing logic
  const { draftBox, handleMouseDown, handleMouseMove, handleMouseUp, toStageRect } = useBoxDrawing({
    image,
    boxes: allBoxes,
  ...
  const visibleBoxes = useMemo(() => {
    ...
    return BOX_RENDER_ORDER.flatMap((type) => (visible.has(type) ? (boxesByType[type] ?? []) : []));
  ...
  const hoveredBox = useMemo(() => {
    ...
    return (
      visibleBoxes.find((box) => box.id === hoveredBoxRef.id && box.type === hoveredBoxRef.type) ??
      null
    );
  }, [visibleBoxes, hoveredBoxRef]);
  const textIndexMap = useMemo(() => buildTextBoxIndexMap(boxesByType.text), [boxesByType.text]);
  ...
  const renderTooltip = () => {
    ...
    const tooltipWidth = Math.min(MAX_TOOLTIP_WIDTH, Math.max(MIN_TOOLTIP_WIDTH, rect.width));
    ...
    if (tooltipX + bubbleWidth + TOOLTIP_MARGIN > stageSize.width) {
      tooltipX = Math.max(TOOLTIP_MARGIN, stageSize.width - bubbleWidth - TOOLTIP_MARGIN);
    }
  ...
  const renderDraftBox = () => {
    ...
    const stroke = activeBoxType === 'panel' ? '#22c55e' : '#f97316';
    return (
      <Rect
        x={rect.x}
        y={rect.y}
  ...
  const emptySubtitleText = emptySubtitle ?? 'Paste an image (Ctrl+V) to add one.';
    ...
    <div ref={containerRef} className={ui.canvasWrap}>
      <div className={ui.layerPanel}>
        <div className="mb-2 border-b border-slate-800/80 pb-2">
          <div className="text-[10px] uppercase tracking-wide text-slate-400">{pageLabel}</div>
          <div className="max-w-[180px] truncate text-[11px] text-slate-200">{filenameLabel}</div>
        </div>
      ...
      <div className="flex items-center justify-center" style={{ height: rowHeight }}>
        ...
        <button
          ...
          className={`${ui.canvasNavBase} ${hasPrev ? ui.canvasNavEnabled : ui.canvasNavDisabled}`}
        >
          &lt;
        </button>
          ...
          {hasImage ? (
            ...
            >
              <Layer>
                ...
                {visibleBoxes.map((box) => {
                  ...
                  const label = box.type === 'text' ? String(textIndexMap.get(box.id) ?? '') : '';
                  const style =
                    box.type === 'panel'
                      ...
                      <Rect
                        ...
                        onMouseLeave={() =>
                          setHoveredBoxRef((current) =>
                            current?.id === box.id && current?.type === box.type ? null : current,
                          )
                        }
                      />
                      {label && (
            ...
            <div className="w-full h-full flex items-center justify-center text-center">
              <div className={ui.emptyState}>
                ...
                <div className={ui.emptyStateSub}>{emptySubtitleText}</div>
              </div>
            </div>
          )}
        </div>
        ...
        <button
          ...
          disabled={!hasNext}
          style={{ height: rowHeight }}
          className={`${ui.canvasNavBase} ${hasNext ? ui.canvasNavEnabled : ui.canvasNavDisabled}`}
        >
          &gt;
        </button>
      </div>
