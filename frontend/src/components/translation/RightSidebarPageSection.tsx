      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between gap-2">
          <button
            ...
            onClick={onPrev}
            disabled={!hasPrev}
            className={`${ui.button.navBase} ${hasPrev ? ui.button.navEnabled : ui.button.navDisabled}`}
          >
            &lt;- Prev
          </button>
          ...
          >
            {pageCount === 0 && <option value="new">New page</option>}
            {pageNumbers.map((n) => (
              <option key={n} value={n}>
                {`Page ${n}${pageFilenames[n - 1] ? ` - ${pageFilenames[n - 1]}` : ''}`}
              </option>
            ))}
            {pageCount > 0 && <option value="new">New page</option>}
          </select>
          ...
          <button
            ...
            onClick={onNext}
            disabled={!hasNext}
            className={`${ui.button.navBase} ${hasNext ? ui.button.navEnabled : ui.button.navDisabled}`}
          >
            Next -&gt;
          </button>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className={
              ...
              ' ' +
              (isDraftPage || pageCount === 0 ? ui.button.insertDisabled : ui.button.insertEnabled)
            }
          >
            Insert before
          </button>
          <button
            ...
            className={
              ui.button.insertBase +
              ' ' +
              (isDraftPage ? ui.button.insertDisabled : ui.button.insertEnabled)
            }
          >
            Insert after
          </button>
          {isDraftPage && (
            <button type="button" onClick={onCancelDraft} className={ui.button.cancel}>
              Cancel
            </button>
          )}
          <button
            ...
            className={
              ...
              ' ' +
              (canDelete ? ui.button.deleteEnabled : ui.button.deleteDisabled)
            }
          >
            Delete page
          </button>
        ...
        {!isDraftPage && currentPageFilename && (
          <div className={ui.mutedTextTiny}>File: {currentPageFilename}</div>
        )}
      </div>
      {confirmDeleteOpen && (
        <div className={ui.modalOverlay}>
          <div className={ui.modalPanel}>
            <div className={ui.modalTitle}>Delete page?</div>
            <div className={ui.modalText}>
              This removes the page image, boxes, and translations.
              <div className={`mt-1 ${ui.textBodySm}`}>{deleteLabel}</div>
            </div>
