...
export function RightSidebarChatSection({
  ...
  const [streamActivity, setStreamActivity] = useState<ChatStreamActivityEntry[]>([]);
  ...
  useEffect(() => {
    ...
    const loadConfig = async () => {
      try {
        ...
        setSelectedModelId((prev) => prev || config.defaultModel || config.models[0]?.id || '');
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (data.length > 0) {
          setSessions(data);
          setActiveSessionId((prev) => (data.some((item) => item.id === prev) ? prev : data[0].id));
          return;
        }
        const draft = buildDraftSession(volumeId, 'Session 1', selectedModelId || null);
        setSessions([draft]);
  ...
  useEffect(() => {
    const shouldScroll = messages.length > 0 || replying || Boolean(streamingText);
    if (!shouldScroll) {
      return;
    }
    endRef.current?.scrollIntoView({ behavior: 'smooth', block: 'end' });
  }, [messages.length, replying, streamingText]);
  ...
  const handleCreateSession = async () => {
    ...
    const draft =
      existingDraft ??
      buildDraftSession(volumeId, `Session ${sessions.length + 1}`, selectedModelId || null);
    setSessions((prev) => {
  ...
  const handleDeleteSession = async () => {
    ...
    const confirmed = window.confirm(`Delete "${label}"? This removes its chat history.`);
    if (!confirmed) {
      return;
    }
    if (!target.isDraft) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (current?.isDraft) {
        ...
        setSessions((prev) => prev.map((item) => (item.id === current.id ? created : item)));
      ...
      if (typeof window !== 'undefined' && 'EventSource' in window) {
        ...
        source.onmessage = (event) => {
          ...
          try {
            ...
            if (streamType === 'done' && payload.message && typeof payload.message === 'object') {
            ...
            if (streamType === 'error') {
              receivedTerminalRef.current = true;
              const message =
                typeof payload.message === 'string' ? payload.message : 'Streaming failed.';
              setError(message);
              setStreamingText('');
              setReplying(false);
              setIsStreaming(false);
      ...
      if (createdSessionId && !messageCreated) {
        ...
        const draft = buildDraftSession(volumeId, draftTitle, selectedModelId || null);
        setSessions((prev) => {
          const filtered = prev.filter((item) => item.id !== createdSessionId && !item.isDraft);
  ...
  return (
    <div className="h-full flex flex-col gap-3 p-4">
              ...
              >
                {sessions.map((session) => (
                  <option key={session.id} value={session.id}>
                    {session.isDraft ? `${session.title} (draft)` : session.title}
                  </option>
                ))}
              </Select>
            )}
      ...
      {!hasVolume && <div className={ui.mutedTextXs}>Select a volume to start chatting.</div>}
      ...
      <div className="space-y-2">
        {activeSession && <div className={ui.mutedTextMicro}>Active: {activeSession.title}</div>}
        <textarea
          value={input}
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <Button
            ...
            disabled={
              replying
                ? !isStreaming
                : !input.trim() || sending || replying || input.length > maxMessageChars
            }
          >
            {replying ? (isStreaming ? 'Stop' : 'Waiting...') : sending ? 'Sending...' : 'Send'}
          </Button>
        </div>
      </div>
    </div>
