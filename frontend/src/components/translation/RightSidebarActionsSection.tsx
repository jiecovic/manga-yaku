...
export function RightSidebarActionsSection({
  ...
  const enabledBoxDetectionProfiles = boxDetectionProfiles.filter((profile) => profile.enabled);
  const normalizedTask = normalizeBoxType(boxDetectionTask);
  const availableBoxDetectionProfiles = enabledBoxDetectionProfiles.filter((profile) => {
  ...
  const autoDetectDisabled = loadingBoxDetectionProfiles || !hasAvailableBoxDetection;
  const autoDetectReason = loadingBoxDetectionProfiles
    ...
    : !hasAvailableBoxDetection
      ...
      : '';
  useEffect(() => {
    if (selectedProfileId && selectedProfileId !== boxDetectionProfileId) {
      onChangeBoxDetectionProfile(selectedProfileId);
    }
  }, [selectedProfileId, boxDetectionProfileId, onChangeBoxDetectionProfile]);
  ...
  const handleResetVolumeData = async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error && err.message ? err.message : 'Failed to reset volume data.';
      ...
      <div className="space-y-2">
        <Field label="Task" layout="row" labelClassName={ui.label}>
          <Select
            ...
            onChange={(e) => onChangeBoxDetectionTask(e.target.value)}
          >
        ...
        <Field label="Detection" layout="row" labelClassName={ui.label} className="min-w-0">
          {loadingBoxDetectionProfiles && <div className={ui.mutedTextXs}>Loading...</div>}

          {!loadingBoxDetectionProfiles && availableBoxDetectionProfiles.length === 0 && (
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <div className="flex flex-col">
                <div className={ui.mutedTextXs}>No models available for this task.</div>
                <div className={ui.mutedTextXs}>Train a model to enable detection.</div>
              </div>
              <Button
                type="button"
                variant="ghostSmall"
          ...
          {!loadingBoxDetectionProfiles && availableBoxDetectionProfiles.length > 0 && (
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <Select
                ...
                onChange={(e) => onChangeBoxDetectionProfile(e.target.value)}
                className="flex-1 min-w-0"
              >
                {availableBoxDetectionProfiles.map((profile) => (
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button type="button" variant="actionAmber" onClick={onOcrPage}>
            OCR (all boxes)
          </Button>
        </div>

        <div className="flex gap-2">
          <Button type="button" variant="actionEmerald" onClick={onPageTranslationWorkflow}>
            Page translation
          </Button>
        </div>
        ...
        <div className="flex gap-2">
          <Button type="button" variant="actionSlate" onClick={onRefreshPageState}>
            Refresh page
          </Button>

          <Button type="button" variant="actionRed" onClick={onClearBoxes}>
            {normalizedTask === 'panel'
              ? 'Clear panels'
              : normalizedTask === 'face'
                ? 'Clear faces'
        ...
        {/* Row 4: Clear text */}
        <div className="flex gap-2">
          <Button type="button" variant="actionSlateSmall" onClick={onClearOcrText}>
            Clear OCR text
          </Button>

          <Button type="button" variant="actionSlateSmall" onClick={onClearTranslationText}>
            Clear translations
          </Button>
        </div>
