...
function TranslateInner({
  ...
  const canPrev = draftInsert ? draftPrevIndex !== null : pages.length > 0;
  const canNext = draftInsert ? draftNextIndex !== null : pages.length > 0;
  ...
  const currentVolumeName =
    volumes.find((volume) => volume.id === selectedVolumeId)?.name ?? selectedVolumeId;
  ...
  const handlePaste = useCallback(
        ...
        if (item.type.startsWith('image/')) {
          ...
          if (file) {
            if (!selectedVolumeId) {
              ...
              const insertBefore =
                draftInsert?.position === 'before' ? anchor?.filename : undefined;
              const insertAfter = draftInsert?.position === 'after' ? anchor?.filename : undefined;
    ...
    [addPageFromClipboard, clearDraftInsert, draftInsert, resolveDraftAnchor, selectedVolumeId],
  );
      ...
      {pasteCreateOpen && (
        <div className={ui.modalOverlay}>
          <div className={ui.modalPanel}>
            ...
            <div className={ui.modalText}>Enter a name to save your pasted page.</div>
            <div className="mt-3 space-y-2">
              <Field label="Name" labelClassName={ui.labelSmall}>
                <input
                  type="text"
                  value={pasteCreateName}
                  onChange={(e) => setPasteCreateName(e.target.value)}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      e.preventDefault();
              ...
              {pasteCreateError && <div className={ui.warningTextTiny}>{pasteCreateError}</div>}
            </div>
