...
export function formatAttemptEvent(
    ...
    if (event.model_id && event.model_id.trim()) {
    ...
    if (event.error_detail && event.error_detail.trim()) {
...
export function areTasksEqual(a: JobTaskRun[], b: JobTaskRun[]): boolean {
    ...
    for (let idx = 0; idx < a.length; idx += 1) {
        ...
        for (let eventIdx = 0; eventIdx < leftEvents.length; eventIdx += 1) {
            ...
            if (
                (lParams?.max_output_tokens ?? null) !==
                    (rParams?.max_output_tokens ?? null) ||
                (lParams?.reasoning_effort ?? null) !==
                    (rParams?.reasoning_effort ?? null)
            ) {
                return false;
            }
        }
