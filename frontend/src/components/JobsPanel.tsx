...
export function JobsPanel() {
  const { jobs, jobsError, jobsLoading, clearFinished, cancelJob, deleteJob } = useJobs();
  ...
  const hasActiveJobs = useMemo(
    () => sortedJobs.some((job) => job.status === 'running' || job.status === 'queued'),
    [sortedJobs],
  );
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <div className={ui.jobsHeader}>
          ...
          <div className="flex items-center gap-2">
            {jobsLoading && <span className={ui.mutedTextMicro}>Loading...</span>}
            <button type="button" className={ui.jobsButtonSmall} onClick={clearFinished}>
              Clear done
            </button>
          </div>
        </div>
        ...
        {sortedJobs.length > 0 && (
          <ul className={ui.jobsList}>
            {sortedJobs.map((job) => {
              ...
              const renderAsSingleTask = job.type === 'ocr_box' || job.type === 'translate_box';
              ...
              const orderedStageParts = orderedStageIds
                ...
                .map((stageId) => `${stageId} ${formatDurationMs(stageDurations[stageId])}`);
              const extraStageParts = Object.entries(stageDurations)
                ...
                .map(([stageId, stageMs]) => `${stageId} ${formatDurationMs(stageMs)}`);
              ...
              if (
                ...
                (job.type === 'prepare_dataset' || job.type === 'train_model') &&
                payload?.dataset_id
              ) {
              ...
              const singleTaskSummary =
                singleTask && payload?.filename
                  ...
                  : singleTask
                    ...
                    : detail;
              const modelId = payload?.modelId ?? (payload as { model_id?: string })?.model_id;
              ...
              const modelMeta = modelMetaParts.length > 0 ? modelMetaParts.join(' | ') : '';
              const canDelete =
                job.status === 'finished' || job.status === 'failed' || job.status === 'canceled';
              ...
              const singleTaskAttemptEvents =
                singleTask && Array.isArray(singleTask.attempt_events)
                  ? singleTask.attempt_events.slice().sort((a, b) => {
              ...
              const singleTaskAttemptCount = singleTask
                ? Math.max(
                    1,
                    Number.isFinite(Number(singleTask.attempt)) ? Number(singleTask.attempt) : 0,
                    singleTaskAttemptEvents.reduce(
                      (max, event) =>
                        Math.max(
                          max,
                          Number.isFinite(Number(event.attempt)) ? Number(event.attempt) : 0,
                        ),
                      0,
                    ),
                  )
              ...
              return (
                <li key={job.id} className={ui.jobsCard}>
                  ...
                  {modelMeta && (job.type === 'ocr_box' || job.type === 'translate_box') && (
                  ...
                  {(singleTaskMessage || job.message) && (
                    <div className={ui.jobsMessage}>{singleTaskMessage || job.message}</div>
                  )}

                  {job.warnings && job.warnings.length > 0 && (
                    <div className={`mt-1 ${ui.warningTextTiny}`}>{job.warnings.join(' | ')}</div>
                  )}

                  {singleTask && typeof singleTaskAttemptCount === 'number' && (
                    <div className={`mt-1 ${ui.jobsMeta}`}>
                  ...
                  {singleTask && singleTaskAttemptEvents.length > 0 && (
                    <div className="mt-1 space-y-0.5">
                      {singleTaskAttemptEvents.map((event, eventIdx) => (
                        <div key={event.id} className={ui.mutedTextMicro}>
                          {formatAttemptEvent(
                            event,
                            eventIdx > 0 ? singleTaskAttemptEvents[eventIdx - 1] : null,
                            {
                              hasLaterAttempt: eventIdx < singleTaskAttemptEvents.length - 1,
                              taskRunning: String(singleTask.status).trim() === 'running',
                            },
                          )}
                        </div>
                      ))}
                    ...
                    <div className="mt-1">
                      <div className="flex items-center justify-between gap-2">
                        <div className={ui.jobsMeta}>
                          Tasks: {taskCounts.running} running | {taskCounts.queued} queued |{' '}
                          {taskCounts.done} done
                          {' | '}
                          {taskCounts.failed} failed | {taskCounts.canceled} canceled
                      ...
                      {tasksState?.error && (
                        <div className={`mt-1 ${ui.errorTextXs}`}>{tasksState.error}</div>
                      )}
                      {expanded && (
                        <div className="mt-1 max-h-56 overflow-y-auto rounded border border-slate-800 bg-slate-950/50 p-1">
                          ...
                          {tasksState?.loading && taskTotal === 0 && (
                            <div className={`px-1 py-1 ${ui.mutedTextTiny}`}>Loading tasks...</div>
                          )}
                          {taskTotal > 0 && (
                            <ul className="space-y-1">
                              {tasks.map((task) => {
                                ...
                                const attemptEvents = Array.isArray(task.attempt_events)
                                  ? task.attempt_events.slice().sort((a, b) => {
                                      const attemptDelta = a.attempt - b.attempt;
                                      if (attemptDelta !== 0) {
                                        return attemptDelta;
                                      }
                                  ...
                                  : [];
                                const attemptCountFromEvents = attemptEvents.reduce(
                                  (max, event) =>
                                    Math.max(
                                      max,
                                ...
                                const progressDisplay = getProgressDisplay(
                                  taskProgressValue(String(task.status)),
                                );
                                return (
                                  <li key={task.id} className={ui.jobsCard}>
                                    <div className="mb-0.5 flex items-center justify-between gap-2">
                                      <span className={ui.jobsType}>{taskTypeLabel(task)}</span>
                                      <span className={taskStatusBadgeClass(String(task.status))}>
                                        {task.status}
                                      </span>
                                    </div>
                                    <div className={ui.jobsDetail}>{summary}</div>
                                    {message && <div className={ui.jobsMessage}>{message}</div>}
                                    <div className={`mt-1 ${ui.jobsMeta}`}>
                                    ...
                                    {attemptEvents.length > 0 && (
                                      <div className="mt-1 space-y-0.5">
                                        {attemptEvents.map((event, eventIdx) => (
                                          <div key={event.id} className={ui.mutedTextMicro}>
                                            {formatAttemptEvent(
                                              event,
                                              eventIdx > 0 ? attemptEvents[eventIdx - 1] : null,
                                              {
                                                hasLaterAttempt:
                                                  eventIdx < attemptEvents.length - 1,
                                                taskRunning:
