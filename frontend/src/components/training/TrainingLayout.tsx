...
import { createPrepareDatasetJob, createTrainModelJob, fetchTrainingModels } from '../../api';
...
export function TrainingLayout() {
  ...
  const [selectedSources, setSelectedSources] = useState<Set<string>>(() => new Set<string>());
  ...
  const [modelFamilies, setModelFamilies] = useState<string[]>(['yolo11', 'yolo12', 'yolo26']);
  const [ultralyticsVersion, setUltralyticsVersion] = useState<string | null>(null);
    ...
    const loadModels = async () => {
      try {
        ...
        if (Array.isArray(response.families) && response.families.length) {
          ...
          setModelFamily((prev) =>
            response.families.includes(prev) ? prev : response.families[0],
          );
        }
      } catch (err) {
  ...
  const availableSources = useMemo(() => sources.filter((source) => source.available), [sources]);
  ...
  const supportedTargets = useMemo(() => {
    ...
    const pool = selectedSourceList.length ? selectedSourceList : availableSources;
  ...
  const { lines: logLines, status: logStatus, error: logError } = useJobLogs(selectedLogJobId);
  ...
  useEffect(() => {
    ...
    if (selectedLogJobId) {
      const exists = trainingJobs.some((job) => job.id === selectedLogJobId);
      if (exists) {
        return;
      }
    }
  ...
  const selectAll = () => {
    setSelectedSources(new Set(availableSources.map((source) => source.id)));
  };
    ...
    <div className="flex-1 flex overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        <main className={ui.trainingMain}>
          <div role="tablist" className={ui.trainingTabs}>
            <button
              type="button"
              role="tab"
              ...
              className={`${ui.trainingTab} ${
                activeTab === 'prep' ? ui.trainingTabActive : ui.trainingTabInactive
              }`}
            >
              Data Preparation
              <span className={`ml-2 ${ui.trainingMeta}`}>{selectedCount} sources</span>
            </button>
            <button
              type="button"
              role="tab"
              ...
              className={`${ui.trainingTab} ${
                activeTab === 'train' ? ui.trainingTabActive : ui.trainingTabInactive
              }`}
            >
              Training
              <span className={`ml-2 ${ui.trainingMeta}`}>{datasets.length} datasets</span>
            </button>
          </div>
          ...
          {activeTab === 'prep' ? (
            <section className={`${ui.trainingSection} space-y-4`}>
              <div className="flex items-center justify-between">
                <h2 className={ui.trainingSectionTitle}>Data Preparation</h2>
                <span className={ui.trainingSectionMeta}>Selected sources: {selectedCount}</span>
              </div>
            ...
            <section className={`${ui.trainingSection} space-y-4`}>
              <div className="flex items-center justify-between">
                <h2 className={ui.trainingSectionTitle}>Training</h2>
                <span className={ui.trainingSectionMeta}>Datasets: {datasets.length}</span>
              </div>
