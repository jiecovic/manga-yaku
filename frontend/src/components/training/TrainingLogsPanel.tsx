...
const clampHistory = (values: MetricPoint[]): MetricPoint[] =>
    values.length > MAX_HISTORY_POINTS
        ? values.slice(values.length - MAX_HISTORY_POINTS)
        : values;
...
const buildMetricHistory = (
    ...
    for (const entry of progressEntries) {
        perEpochTotals.set(
            entry.epoch,
            (perEpochTotals.get(entry.epoch) ?? 0) + 1,
        );
    }
    ...
    if (points.length < 2) {
        return (
            <div className="rounded-md border border-slate-800 bg-slate-950/60 p-2">
                ...
                <div className={`mt-2 ${ui.mutedTextTiny}`}>
                    Waiting for metrics...
                </div>
            </div>
        );
...
export function TrainingLogsPanel({
    ...
    const selectedLogJob =
        trainingJobs.find((job) => job.id === selectedLogJobId) ?? null;
    ...
    return (
        <section className={ui.trainingSectionCompact}>
            <div className="flex items-center justify-between">
                ...
                <span className={ui.trainingLogBadge}>
                    {logStatusLabel}
                </span>
            </div>
            {trainingJobs.length === 0 ? (
                <EmptyState className="mt-3 h-24">
                    Start a training run to see logs.
                </EmptyState>
            ) : (
                <div className="mt-3 space-y-2">
                    <Field label="Log source" labelClassName={ui.trainingLabelSmall}>
                        <Select
                            ...
                            onChange={(event) =>
                                onSelectLogJob(event.target.value || null)
                            }
                        >
                            <option value="">Select a training job</option>
                            {trainingJobs.map((job) => {
                                const payload =
                                    job.payload as TrainingJobPayload;
                        ...
                        <div className={ui.trainingCardCompact}>
                            ...
                            {progressDisplay.progress !== null && (
                                <div className="mt-2">
                                    ...
                                    <div className={`mt-1 ${ui.trainingLabelTiny}`}>
                            ...
                            {displayMetrics && (
                                <div className={`mt-3 grid grid-cols-2 gap-2 ${ui.trainingLogMetrics}`}>
                                    <div>
                                        Epoch: {displayMetrics.epoch ?? "?"}/
                                        {displayMetrics.total_epochs ?? "?"}
                                    </div>
                                    {displayMetrics.batch !== null &&
                                        ...
                                        displayMetrics.batches !== undefined && (
                                            <div>
                                                Batch: {displayMetrics.batch}/
                                                {displayMetrics.batches}
                                            </div>
                                        )}
                                    {displayMetrics.device && (
                                        <div>
                                            Device: {displayMetrics.device}
                                        </div>
                                    )}
                                    {displayMetrics.gpu_mem && (
                                        <div>
                                            GPU mem: {displayMetrics.gpu_mem}
                                        </div>
                                    )}
                                    {displayMetrics.box_loss !== null &&
                                        displayMetrics.box_loss !== undefined && (
                                            <div>
                                                Box loss:{" "}
                                                {formatFloat(
                                                    displayMetrics.box_loss,
                                                )}
                                            </div>
                                        )}
                                    {displayMetrics.cls_loss !== null &&
                                        displayMetrics.cls_loss !== undefined && (
                                            <div>
                                                Cls loss:{" "}
                                                {formatFloat(
                                                    displayMetrics.cls_loss,
                                                )}
                                            </div>
                                        )}
                                    {displayMetrics.dfl_loss !== null &&
                                        displayMetrics.dfl_loss !== undefined && (
                                            <div>
                                                DFL loss:{" "}
                                                {formatFloat(
                                                    displayMetrics.dfl_loss,
                                                )}
                                            </div>
                                        )}
                                    {displayMetrics.map50 !== null &&
                                        displayMetrics.map50 !== undefined && (
                                            <div>
                                                mAP50:{" "}
                                                {formatFloat(
                                                    displayMetrics.map50,
                                                )}
                                            </div>
                                        )}
                                    {displayMetrics.map50_95 !== null &&
                                        displayMetrics.map50_95 !== undefined && (
                                            <div>
                                                mAP50-95:{" "}
                                                {formatFloat(
                                                    displayMetrics.map50_95,
                                                )}
                                            </div>
                                        )}
                                    {displayMetrics.lr !== null &&
                                        displayMetrics.lr !== undefined && (
                                            <div>
                                                LR:{" "}
                                                {formatFloat(
                                                    displayMetrics.lr,
                    ...
                    >
                        {summaryLines.length > 0 ? (
                            ...
                            <div className={`p-3 ${ui.mutedTextTiny}`}>
                                Waiting for updates...
                            </div>
                        )}
                    </div>
                    {selectedLogJob && (
                        <div className={ui.trainingMetaSmall}>
                            Status: {selectedLogJob.status}
                        </div>
                    )}
                    {logError && (
                        <div className={ui.trainingWarningSmall}>
                            {logError}
                        </div>
                    )}
                </div>
