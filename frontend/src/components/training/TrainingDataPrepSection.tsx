    ...
    <>
      <section className={ui.trainingSection}>
        <div className={ui.trainingSectionHeader}>
          <h2 className={ui.trainingSectionTitle}>Sources</h2>
          <div className="flex items-center gap-2">
            <button type="button" className={ui.trainingButton} onClick={refresh}>
              Refresh
            </button>
            <button type="button" className={ui.trainingButton} onClick={selectAll}>
              Select all
            </button>
            <button type="button" className={ui.trainingButton} onClick={clearAll}>
              Clear
            </button>
          </div>
        </div>
        {loading && <div className={ui.trainingHelp}>Loading sources...</div>}
        {error && <div className={ui.trainingError}>{error}</div>}
        {!loading && !error && sources.length === 0 && (
          <div className={ui.trainingHelp}>
            No training sources found yet. Add datasets under training-data/sources.
          </div>
        )}
        {!loading && sources.length > 0 && (
          <div className={ui.trainingList}>
            {sources.map((source) => {
              ...
              const volumes =
                stats?.volumes === null || stats?.volumes === undefined ? 'n/a' : stats.volumes;
              const images =
                stats?.images === null || stats?.images === undefined ? 'n/a' : stats.images;
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <div className={ui.trainingItemTitle}>{source.label}</div>
                      <span className={`${ui.trainingLabelTiny} uppercase tracking-wide`}>
                        {source.type}
                      </span>
                    </div>
                    {source.description && (
                      <div className={`mt-1 ${ui.trainingLabelSmall}`}>{source.description}</div>
                    )}
                    ...
                    {source.path && (
                      <div className={`mt-1 ${ui.trainingLabelSmall}`}>{source.path}</div>
                    )}
                  </div>
                </label>
              );
      ...
      {selectedCount === 0 ? (
        <section className={ui.trainingSectionCompact}>
          <h3 className={ui.trainingSubTitle}>Dataset builder</h3>
          <div className={ui.trainingHelp}>
            Select at least one source to configure dataset preparation.
          </div>
        </section>
      ) : (
        <section className={ui.trainingSectionCompact}>
          ...
          <div className={`mt-3 ${ui.trainingList}`}>
            <label className="block">
              <span className={ui.trainingLabelSmall}>Dataset id</span>
              <input
                value={datasetId}
            ...
            <div className={`${ui.trainingCardCompact} ${ui.trainingListTight}`}>
              {supportedTargets.length === 0 && (
                <div className={ui.trainingHintSmall}>
                  No supported targets found for the selected sources.
                </div>
              )}
            ...
            <label className="block">
              <span className={ui.trainingLabelSmall}>Val split</span>
              <input
                type="number"
                step="0.01"
                min="0"
                max="0.99"
                value={valSplit}
                onChange={(event) => setValSplit(event.target.value)}
                className={ui.trainingInput}
              />
            </label>
            <label className="block">
              <span className={ui.trainingLabelSmall}>Test split</span>
              <input
                type="number"
                step="0.01"
                min="0"
                max="0.99"
                value={testSplit}
                onChange={(event) => setTestSplit(event.target.value)}
                className={ui.trainingInput}
              />
            </label>
            <label className="block">
              <span className={ui.trainingLabelSmall}>Seed</span>
              <input
                type="number"
                step="1"
                min="0"
            ...
            <label className={`flex items-start gap-2 ${ui.trainingMetaSmall}`}>
              <input
                ...
                onChange={(event) => setUseHardlinks(event.target.checked)}
                className="mt-1"
              />
              <span>Use hardlinks (saves space, same drive only)</span>
            </label>
          ...
          {prepareError && <div className={`mt-2 ${ui.trainingError}`}>{prepareError}</div>}
          {splitError && <div className={`mt-2 ${ui.trainingWarning}`}>{splitError}</div>}
          {prepareJobId && (
