...
class PageTranslationRequest:
    ...
    @classmethod
    def from_payload(cls, payload: dict[str, Any]) -> PageTranslationRequest:
...
@dataclass(frozen=True)
class PageTranslationWorkflowSnapshot:
    state: WorkflowState
    stage: str
    progress: int
    message: str
...
ProgressCallback = Callable[[PageTranslationWorkflowSnapshot], None]
